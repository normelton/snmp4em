...
describe "When performing a single SNMPv2 GETBULK request" do
  it "should fetch the next ten values" do
    @snmp_v2.getbulk("1.9.9.5.1").expect do |response|
      response.should have(1).item
      response["1.9.9.5.1"].should == {
        "1.9.9.5.1.1" => "1-A",
        "1.9.9.5.1.2" => "1-B",
        "1.9.9.5.1.3" => "1-C",
        "1.9.9.5.1.4" => "1-D",
  ...
  it "should fetch four values when limited" do
    @snmp_v2.getbulk("1.9.9.5.1", :max_results => 4).expect do |response|
      response.should have(1).item
      response["1.9.9.5.1"].should == {
        "1.9.9.5.1.1" => "1-A",
        "1.9.9.5.1.2" => "1-B",
  ...
  it "should fetch only four values if it reaches the end of the MIB" do
    @snmp_v2.getbulk("1.9.9.5.3").expect do |response|
      response.should have(1).item
      response["1.9.9.5.3"].should == {
        "1.9.9.5.3.1" => "3-A",
        "1.9.9.5.3.2" => "3-B",
        "1.9.9.5.3.3" => "3-C",
        "1.9.9.5.3.4" => "3-D"
...
describe "When performing multiple SNMPv2 GETBULK requests simultaneously" do
  it "should fetch two values correctly" do
    @snmp_v2.getbulk(["1.9.9.5.1", "1.9.9.5.2"]).expect do |response|
  ...
  it "should honor the non_repeaters value" do
    @snmp_v2.getbulk(["1.9.9.5.1", "1.9.9.5.2"], :non_repeaters => 1).expect do |response|
      response.should have(2).items
